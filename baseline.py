...
def get_ori(image,postion,window_size):
    ...
    x = np.floor(postion[0])-np.floor(window_size[0]/2) + np.arange(window_size[0], dtype=int)
    y = np.floor(postion[1])-np.floor(window_size[1]/2) + np.arange(window_size[1], dtype=int)
...
def tracking(ori,num,den,label_size):
    ...
    return center_w,center_h,final_response.shape
...
def translate_img_center(w,h,map_size,position):

    w_delta, h_delta = [(w - map_size[1]/2)*cell_size, (h - map_size[0]/2)*cell_size]
...
for index in range(len(image_file)):
    ...
    tracking_imsize = (tracking_image.shape[0], tracking_image.shape[1])

    tracking_ori = get_ori(tracking_image, position, window_size)

    center_w, center_h, response_shape = tracking(tracking_ori, num, den, filter_shape)
    position = translate_img_center(center_w, center_h, response_shape, position)
    ...
    traget = (position[0]-size[0]/2,position[1]-size[1]/2,position[0]+size[0]/2,position[1]+size[1]/2)
