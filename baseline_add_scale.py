...
def get_ori(image,postion,wsz,scale_factor = None):

    sz_ori = wsz

    patch_wsz = wsz
    if scale_factor != None:
        patch_wsz = np.floor(wsz*scale_factor)

    x = np.floor(postion[0])-np.floor(patch_wsz[0]/2) + np.arange(patch_wsz[0], dtype=int)
    y = np.floor(postion[1])-np.floor(patch_wsz[1]/2) + np.arange(patch_wsz[1], dtype=int)
...
def tracking(image,position,size,window_size,num,den,label_size,s_num,s_den,current_scale_factor,scaleFactors,scale_window,model_size):

    #window_size no change
    ori = get_ori(image,position,window_size,current_scale_factor)
    ...
    w_delta, h_delta = [(center_w - final_response.shape[1]/2)*current_scale_factor*cell_size, (center_h - final_response.shape[0]/2)*current_scale_factor*cell_size]

    center = [position[0] + w_delta, position[1] + h_delta]

    sw = get_scale_window(image,center,size,current_scale_factor*scaleFactors,scale_window,model_size)
    swf = np.fft.fftn(sw,axes=[0])
    scale_response = np.real(np.fft.ifftn(np.sum(np.divide(np.multiply(s_num,swf),(s_den[:,None] + lam)),axis=1)))
    scale_index = np.argmax(scale_response)
    current_scale_factor = current_scale_factor * scaleFactors[scale_index]
    ...
    target_size = size * current_scale_factor

    return center,target_size,current_scale_factor

def update_filter(image,position,size,window_size,yf,cos_window,num,den,update_rate,
                  s_num, s_den, current_scale_factor, scaleFactors, scale_window, model_size):
    ...
    sw = get_scale_window(image, position, size, current_scale_factor*scaleFactors, scale_window, model_size)
    swf = np.fft.fftn(sw, axes=[0])
    new_s_num = np.multiply(ysf[:, None], np.conj(swf))
    new_s_den = np.real(np.sum(np.multiply(swf, np.conj(swf)), axis=1))
    s_num = (1 - update_rate)*s_num + update_rate*new_s_num
    s_den = (1 - update_rate)*s_den + update_rate*new_s_den

    return num,den,s_num,s_den
...
size = np.array(size)
window_size = get_search_windows(size,im_size)
...
min_scale_factor = np.power(scale_step,np.ceil(np.log(5./np.min(window_size))/ np.log(scale_step)))
max_scale_factor = np.power(scale_step,np.floor(np.log(np.min(np.divide(initial_frame.shape[:2],size)))/np.log(scale_step)))
...
for index in range(len(image_file)):
    ...
    position, target_size, current_scale_factor = tracking(tracking_image,position,size,window_size,num,den,filter_shape,s_num,s_den,current_scale_factor,scaleFactors,scale_window,scale_model_sz)

    num, den, s_num, s_den = update_filter(tracking_image,position,size,window_size,yf,cos_window,num,den,0.01,
                  s_num, s_den, current_scale_factor, scaleFactors, scale_window, scale_model_sz)
