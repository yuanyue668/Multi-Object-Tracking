...
def get_search_windows_test(size,im_size):
...
def get_scale_window_test(image,position,target_size,sfs,scale_window,scale_model_sz):
...
def get_scale_filter(image,position,target_size,current_scale_factor,scaleFactors,scale_window,model_size,ysf):
    ...
    sw = get_scale_window_test(image,position,target_size,current_scale_factor*scaleFactors,scale_window,model_size)
...
def scale_variation(image,target_position,target_size,scale_num,scale_den,scale_factor,ScaleFactors,scale_window,model_size):
    ...
    sw = get_scale_window_test(image,target_position,target_size,scale_factor*ScaleFactors,scale_window,model_size)
    swf = np.fft.fftn(sw,axes=[0])
...
class tracker:
    def __init__(self, image, size, position):
        ...
        self.sz = get_search_windows_test(self.target_size, image.shape[:2])
